...
def detect_face(img,gts,imgpath,p_model):
    ...
    for scale in scales:
        ...
        print(scale)
        img_x = transform.resize(img,(hs,ws),interpolation=cv2.INTER_LINEAR)
        ...
        print('boxed shape:',boxes.shape[0])
        if boxes.shape[0] != 0:
            #print(boxes[4:9])
        ...
        print('total_boxes.shape:',total_boxes.shape[0])
    ...
    if numbox > 0:
        ...
        for k in range(numbox):
            ...
            if(Iou >= 0.65):
                ...
                global n_idx
                #resized_img = cv2.resize(crop_img,(stdsize,stdsize), interpolation=cv2.INTER_LINEAR)
