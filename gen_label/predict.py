    ...
    if numbox > 0:
        ...
        if total_boxes.shape[0] > 0:
            ...
            if len(pick) > 0 :
                ...
                p = open('/lfs1/users/szhu/project/MTCNN-keras/rnet_out_nms.txt','w')
                t1 = total_boxes[:,0]
                t2 = total_boxes[:,1]
                t3 = total_boxes[:,2]
                t4 = total_boxes[:,3]
        ...
        if numbox > 0:
            ...
            mv = out[1][pass_t, :].T
            lm = out[2][pass_t, :]
            lm[:,0:9:2] = np.tile(w, (5,1)).T * lm[:,0:9:2] + np.tile(total_boxes[:,0], (5,1)).T - 1
            lm[:,1:10:2] = np.tile(w, (5,1)).T * lm[:,1:10:2] + np.tile(total_boxes[:,1], (5,1)).T - 1
            
            if total_boxes.shape[0] > 0:

                total_boxes = bbreg(total_boxes, mv[:,:])
                p = open('/lfs1/users/szhu/project/MTCNN-keras/onet_out_no.txt','w')
                t1 = total_boxes[:,0]
                t2 = total_boxes[:,1]
                t3 = total_boxes[:,2]
                t4 = total_boxes[:,3]
                ...
                if len(pick) > 0 :
                    ...
                    p = open('/lfs1/users/szhu/project/MTCNN-keras/onet_out.txt','w')
                    t1 = total_boxes[:,0]
                    t2 = total_boxes[:,1]
                    t3 = total_boxes[:,2]
                    t4 = total_boxes[:,3]
                    ...
                    p.close()
                    lm = lm[pick, :]
                    p = open('/lfs1/users/szhu/project/MTCNN-keras/landmark_out.txt','w')
                    for i in range(lm.shape[0]):
                        for j in range(lm[i].shape[0]):
                            p.write(str(lm[i][j]))
                            p.write(' ')
                        p.write('\n')
                    p.close()
...
# 初始化PNet
p_model = PNet_p.PNet()
p_model.load_weights('/lfs1/users/szhu/project/MTCNN-keras/model/real_accuracy/pnet.h5')
r_model= PNet.RNet()
r_model.load_weights('/lfs1/users/szhu/project/MTCNN-keras/model/real_accuracy/rnet.h5')
o_model = PNet.ONet()
o_model.load_weights('/lfs1/users/szhu/project/MTCNN-keras/model/real_accuracy/onet.h5')
...
#image_path = '/lfs1/users/szhu/project/MTCNN-keras/gen_label/test1.jpg'
