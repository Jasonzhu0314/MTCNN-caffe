...
class LossHistory(keras.callbacks.Callback):
    def __init__(self, bloss_path,eloss_path,vloss_path):
        #super(LossHistory, self).__init__()
        self.bloss_path = bloss_path
        self.eloss_path = eloss_path
        self.vloss_path = vloss_path
    ...
    def on_batch_end(self, batch, logs={}):
        with open(self.bloss_path,'a') as batch_loss:
            batch_loss.write(str(logs.get('loss')))
            batch_loss.write(' ')
            batch_loss.write(str(logs.get('acc')))
            batch_loss.write('\n')
    def on_epoch_end(self, batch, logs={}):
        with open(self.eloss_path,'a') as epoch_loss:       
            epoch_loss.write(str(logs.get('loss')))
            epoch_loss.write(' ')
            epoch_loss.write(str(logs.get('acc')))
            epoch_loss.write('\n')   
        with open(self.vloss_path,'a') as v_loss:
            v_loss.write(str(logs.get('val_loss')))
            v_loss.write(' ')
            v_loss.write(str(logs.get('val_acc')))
            v_loss.write('\n')
