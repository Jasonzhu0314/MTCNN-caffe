...
modelpath ='/lfs1/users/szhu/project/MTCNN-keras/model/real_accuracy/r0.001-1-{epoch:02d}.h5'
log_filepath = '/lfs1/users/szhu/project/MTCNN-keras/loss/sgdr0.001-1'
batch_size = 80
num_classes = 3
nb_epoch = 20
train_path = './data4/24/label-train.txt'
test_path = './data4/24/label-test.txt'
model = RNet()
model.load_weights('/lfs1/users/szhu/project/MTCNN-keras/model/real_accuracy/r0.01-1-19.h5')
...
model.fit_generator(generate_arrays_from_file(train_path,batch_size),  
                        steps_per_epoch=8083,nb_epoch=nb_epoch,
                        validation_data=generate_arrays_from_file(test_path,batch_size),
                        validation_steps=899,callbacks=[model_per_epoch,tb_cb])
