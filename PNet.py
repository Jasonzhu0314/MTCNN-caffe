...
def PNet():
    image_input = Input(shape=(12,12,3))
...
def RNet():
    image_input = Input(24,24,3)
    xavier = initializer.glorot_uniform()
    x = Conv2D(28,(3,3),kernel_initializer=xavier, name='conv1')(image_input)
    x = PReLU(name='prelu1')(x)
    x = MaxPooling2D((3,3),strides=(2,2), name='pool1')(x)

    x = Conv2D(48,(3,3),kernel_initializer=xavier, name='conv2')
    x = PReLU(name='prelu2')(x)
    x = MaxPooling2D((3,3),strides=(2,2), name='pool2')(x)
   
    x = Conv2D(64,(2,2),kernel_initializer=xavier, name='conv3')
    x = PReLU(name='prelu3')(x)
    ...
    cla = Dense(2,activation=softmax,kernel_initializer=xavier,name='cla')(x)
...
def ONet():
    ...
    x = Conv2D(64,(3,3),kernel_initializer=xavier, name='conv2')
    x = PReLU(name='prelu2')(x)
    x = MaxPooling2D((3,3),strides=(2,2), name='pool2')(x)
   
    x = Conv2D(64,(3,3),kernel_initializer=xavier, name='conv3')
    x = PReLU(name='prelu3')(x)
    x = MaxPooling2D((2,2),strides=(2,2), name='pool3')

    x = Conv2D(128,(2,2),kernel_initializer=xavier, name='conv4')
    x = PReLU(name='prelu4')(x)
